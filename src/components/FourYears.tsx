  ...
  return (
    <div className="space-y-6 w-full">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pb-6 border-b border-white/10">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-white tracking-tight">The Four-Year Cycle</h2>
          <p className="text-[#888] text-sm mt-1">Fasika shift across the Evangelists</p>
        </div>
        <div className="flex items-center gap-4">
          <button onClick={() => setBaseYear(y => y - 4)} className="px-3.5 py-2 bg-[#111] border border-white/10 rounded-md hover:bg-white/5 transition text-sm text-[#888] hover:text-white">
            Previous Cycle
          </button>
          <button onClick={() => setBaseYear(y => y + 4)} className="px-3.5 py-2 bg-[#111] border border-white/10 rounded-md hover:bg-white/5 transition text-sm text-[#888] hover:text-white">
            Next Cycle
        ...
        {cycleYears.map((year, i) => {
          ...
          return (
            <div key={year} className="bg-[#050505] rounded-xl border border-white/10 overflow-hidden flex flex-col hover:border-white/20 transition-colors">
              <div className="p-5 bg-[#111] border-b border-white/10 text-center">
                <div className="text-xl font-ethiopic font-bold text-white mb-1">
                  {evangelist.name}
                </div>
              ...
              <div className="p-5 flex-1 flex flex-col justify-center space-y-6">
                <div className="text-center">
                  ...
                  <div className="text-xl font-ethiopic text-white font-bold">
                    {ethiopianMonths[fasika.month - 1]} {fasika.day}
                  </div>
                </div>

                <div className="space-y-2.5">
                  <div className="flex justify-between text-[11px] text-[#888] font-ethiopic">
                    <span>መጋቢት 26</span>
                    <span>ሚያዝያ 30</span>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4 text-center text-xs pt-5 border-t border-white/10">
                  <div>
