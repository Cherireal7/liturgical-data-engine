...
import { ethiopianMonths, gcToEt, etToGc, calculateBahireHasab, fixedFeasts, fixedFasts } from "@/engine/bahireHasab";
...
export default function Converter() {
  ...
  const [gcDate, setGcDate] = useState(todayGc.toISOString().split('T')[0]);

  const [lastEdited, setLastEdited] = useState<"ET" | "GC">("ET");

  useEffect(() => {
    if (lastEdited === "ET") {
      try {
        const d = etToGc(etDate.year, etDate.month, etDate.day);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 w-full">
      <div className="text-center space-y-4">
        <h2 className="text-3xl md:text-5xl font-bold text-white tracking-tighter">Real-Time Converter</h2>
        <p className="text-[#888] max-w-lg mx-auto text-lg">
          Convert between the Gregorian and Ethiopian calendars instantly, while maintaining full liturgical context.
        </p>
      </div>

      <div className="relative flex flex-col md:flex-row items-center gap-8 p-8 border border-white/10 rounded-2xl bg-[#050505]">
        
        {/* Ethiopian Input */}
        <div className="flex-1 w-full space-y-6">
          <div className="flex items-center gap-2 text-white pb-4 border-b border-white/10">
            <Calendar className="w-5 h-5 text-[#888]" />
            <h3 className="font-semibold text-lg tracking-tight">Ethiopian Date</h3>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <input 
                ...
                className="w-full bg-[#111] border border-white/10 rounded-md px-3 py-2 text-white text-center focus:outline-none focus:border-white transition-colors"
              />
            </div>
            <div>
              ...
              <select 
                ...
                className="w-full bg-[#111] border border-white/10 rounded-md px-2 py-2 text-white text-center focus:outline-none focus:border-white font-ethiopic transition-colors"
              >
          ...
          <div className="text-center pt-4">
            <span className="text-white font-ethiopic text-2xl font-bold tracking-tight">
        ...
        {/* Gregorian Input */}
        <div className="flex-1 w-full space-y-6">
          <div className="flex items-center justify-end gap-2 text-white pb-4 border-b border-white/10">
            <h3 className="font-semibold text-lg tracking-tight">Gregorian Date</h3>
            <Calendar className="w-5 h-5 text-[#888]" />
          </div>
          ...
          <div>
            ...
            <input 
              ...
              className="w-full bg-[#111] border border-white/10 rounded-md px-4 py-3 text-white text-center focus:outline-none focus:border-white text-lg tracking-widest [color-scheme:dark] transition-colors"
            />
          </div>
      ...
      <div className="border border-white/10 rounded-2xl p-8 max-w-2xl mx-auto text-center bg-[#050505]">
        <h3 className="text-xs uppercase tracking-widest text-[#888] font-bold mb-6">Liturgical Context</h3>
        ...
        {context.feasts.length === 0 && context.fasts.length === 0 ? (
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {context.feasts.map((f, i) => (
              <div key={i} className="px-4 py-2 bg-white text-black border border-white rounded-md font-ethiopic font-semibold shadow-sm">
                {f}
              </div>
            ))}
            {context.fasts.map((f, i) => (
              <div key={i} className="px-4 py-2 bg-[#111] border border-white/10 rounded-md text-[#888] font-ethiopic font-medium">
                {f}
              </div>
            ))}
          </div>
